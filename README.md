...
Чтобы сгенерировать gRPC-клиентские классы из `.proto`, запусти `./gradlew :analytics-proto:generateProto` — артефакты будут доступны для тестов через модуль `analytics-proto`.
